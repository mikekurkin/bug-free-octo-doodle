import type { City, Game, GameResult, PaginatedResponse, RankMapping, Series, Team } from '../types/data';
...
export class MockStorage implements Storage {
  ...
  async getGames(
    ...
    try {
      ...
      if (params.cityId) {
        filteredGames = filteredGames.filter(game => game.city_id === params.cityId);
      }
      ...
      if (params.search) {
        ...
        filteredGames = filteredGames.filter(
          game =>
            game._id.toString().includes(search) ||
            game.number.toLowerCase().includes(search) ||
      ...
      throw new StorageError('Failed to get games', error);
    }
  }
