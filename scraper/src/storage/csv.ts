...
export class CsvStorage implements Storage {
  ...
  constructor(
    gamesFile = 'data/games.csv',
    resultsFile = 'data/results.csv',
    citiesFile = 'data/cities.csv',
    ranksFile = 'data/ranks.csv',
    teamsFile = 'data/teams.csv',
    seriesFile = 'data/series.csv'
  ) {
  ...
  async saveGames(games: Game[]): Promise<void> {
    ...
    try {
      ...
      await csvWriter.writeRecords([...gamesWithProcessedFlag].sort((a, b) => Number(a.date) - Number(b.date)));
